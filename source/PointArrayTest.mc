...
function latLon2xyTest(logger as Logger) as Boolean {
    var point = RectangularPoint.latLon2xy(-27.49225, 153.030049);
    logger.debug("point = " + point);
    Test.assert(point != null);
    if (point == null) {
        return false;
    }
    return point.x == 17035226.0 && point.y == -3185107.25;
}

(:test,:debug)
function latLon2xyTest2(logger as Logger) as Boolean {
    var point = RectangularPoint.latLon2xy(-26.49225, 153.030049);
    logger.debug("point = " + point);
    Test.assert(point != null);
    if (point == null) {
        return false;
    }
    return point.x == 17035226.0 && point.y == -3060177.5;
}
...
function latLon2xyRoundTrip(logger as Logger) as Boolean {
    var lat = -26.49225;
    var long = 153.030049;
    var point = RectangularPoint.latLon2xy(lat, long);
    Test.assert(point != null);
    if (point == null) {
        return false;
    }
...
function restrictPointsToMaxMemoryLessThanHalf(logger as Logger) as Boolean {
    ...
    points.add(new RectangularPoint(1f,2f));
    points.add(new RectangularPoint(4f,5f));
    points.add(new RectangularPoint(7f,8f));
    points.add(new RectangularPoint(10f,11f));
    points.add(new RectangularPoint(13f,14f));
    points.add(new RectangularPoint(16f,17f));
    ...
    Test.assertEqual(points.size(), 6);

    Test.assertEqual(points._internalArrayBuffer[0], 1f);
    Test.assertEqual(points._internalArrayBuffer[1], 2f);
    Test.assertEqual(points._internalArrayBuffer[2], 10f);
    Test.assertEqual(points._internalArrayBuffer[3], 11f);
    Test.assertEqual(points._internalArrayBuffer[4], 16f);
    Test.assertEqual(points._internalArrayBuffer[5], 17f);
...
function restrictPointsOddNumberKeepsLastPointTest(logger as Logger) as Boolean {
    ...
    Test.assertEqual(points.size(), 8);

    Test.assertEqual(points._internalArrayBuffer[0], 1f);
    Test.assertEqual(points._internalArrayBuffer[1], 2f);
    Test.assertEqual(points._internalArrayBuffer[2], 7f);
    Test.assertEqual(points._internalArrayBuffer[3], 8f);
    Test.assertEqual(points._internalArrayBuffer[4], 13f);
    Test.assertEqual(points._internalArrayBuffer[5], 14f);
...
function restrictPointsOddNumberKeepsLastPointEvenIfOddTest(logger as Logger) as Boolean {
    ...
    Test.assertEqual(points.size(), 10);
